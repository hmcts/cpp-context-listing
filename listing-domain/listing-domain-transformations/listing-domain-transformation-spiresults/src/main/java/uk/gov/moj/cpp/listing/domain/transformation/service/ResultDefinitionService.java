...
import uk.gov.moj.cpp.listing.domain.transformation.helper.JDBCConnectionFactory;
...
public class ResultDefinitionService {
    ...
    public ResultDefinition getResultDefinition(final String eventDate, final String label) {
        ...
        try (final Connection connection = new JDBCConnectionFactory().getConnection();
            ...
            LOGGER.error("Result definition not found label {} and date {}", label, eventDate);
            throw new TransformationException(format("Result definition not found label '%s' and date '%s'", label, eventDate));
