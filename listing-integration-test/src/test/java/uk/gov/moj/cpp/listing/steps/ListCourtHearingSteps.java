...
public class ListCourtHearingSteps extends AbstractIT implements AutoCloseable {
    ...
    private void stubReferenceDataForFirstHearing() {
        ...
        hearingsData.getHearingData().stream()
                ...
                .forEach(cci -> {
                    ...
                    stubGetReferenceDataCourtRoomMappings(new CourtCentreData(cci, DEFAULT_START_TIME, DEFAULT_DURATION_HOURS_MINS, hearingsData.getHearingData().get(0).getCourtRoomId()));
                });
    ...
    private String generateUrlForFindingAHearingById(String rawId) {
        return String.format("%s/%s", baseUri,
                format(ENDPOINT_PROPERTIES.getProperty("listing.search.hearing"),
                        rawId
                ));
    }
