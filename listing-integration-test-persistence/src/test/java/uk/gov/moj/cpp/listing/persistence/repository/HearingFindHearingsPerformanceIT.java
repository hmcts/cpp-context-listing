 ...
 *
 * Disabled by default so it never slows down the regular IT suite - remove @Disabled locally,
 * point at a real Postgres instance, and run it before/after SQL or index changes to compare
 * timings printed to stdout.
 *
 * Ported from the Java 17 line's DeltaSpike CdiTestRunner to this branch's JUnit 5 +
 * HibernateTestEntityManagerProvider pattern (see PersistenceTestsIT), since the DeltaSpike
 * test-control modules were removed by the DeltaSpike -> JPA migration.
 */
@Disabled("Manual performance test - remove @Disabled locally to run against a real DB with bulk data")
public class HearingFindHearingsPerformanceIT {
    ...
    @BeforeEach
    public void createRepositoryAndClearDatabase() {
    ...
    /**
     * Mirrors the helper in {@link PersistenceTestsIT}. The Java 17 line used Hibernate Types'
     * {@code JacksonUtil}, which this branch no longer pulls in.
     */
    private static JsonNode toJsonNode(final String json) {
        try {
            return OBJECT_MAPPER.readTree(json);
        } catch (final IOException e) {
            throw new IllegalArgumentException("Could not read '" + json + "' as JSON", e);
        }
    }
}
