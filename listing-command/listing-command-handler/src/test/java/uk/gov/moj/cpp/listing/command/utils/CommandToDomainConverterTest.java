...
class CommandToDomainConverterTest {
    ...
    @Test
    void shouldReturnListedStartDateTimeWhenPresent() {
        final HearingListingNeeds commandHearing = commandBuilder.buildHearingWithListedStartDateTime();

        final ZonedDateTime result = CommandToDomainConverter.getStartDateTime(commandHearing);

        assertThat(result, is(commandHearing.getListedStartDateTime()));
    }

    @Test
    void shouldReturnEarliestStartDateTimeWhenListedStartDateTimeIsNull() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearing();

        final ZonedDateTime result = CommandToDomainConverter.getStartDateTime(commandHearing);

        assertThat(result, is(commandHearing.getEarliestStartDateTime()));
    }

    @Test
    void shouldBuildCommittingCourtFromOffenceViaConvert() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearingWithCommittingCourt();

        final uk.gov.moj.cpp.listing.domain.Hearing actual = commandToDomainConverter.convert(commandHearing);

        final uk.gov.justice.core.courts.CommittingCourt commandCommittingCourt =
                commandHearing.getProsecutionCases().get(0).getDefendants().get(0).getOffences().get(0).getCommittingCourt();
        final uk.gov.moj.cpp.listing.domain.CommittingCourt domainCommittingCourt =
                actual.getListedCases().get(0).getDefendants().get(0).getOffences().get(0).getCommittingCourt().get();

        assertThat(domainCommittingCourt.getCourtCentreId(), is(commandCommittingCourt.getCourtCentreId()));
        assertThat(domainCommittingCourt.getCourtHouseName(), is(commandCommittingCourt.getCourtHouseName()));
        assertThat(domainCommittingCourt.getCourtHouseCode().get(), is(commandCommittingCourt.getCourtHouseCode()));
        assertThat(domainCommittingCourt.getCourtHouseShortName().get(), is(commandCommittingCourt.getCourtHouseShortName()));
    }

    @Test
    void shouldReturnEmptyCommittingCourtWhenOffenceHasNoCommittingCourt() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearing();

        final uk.gov.moj.cpp.listing.domain.Hearing actual = commandToDomainConverter.convert(commandHearing);

        assertThat(actual.getListedCases().get(0).getDefendants().get(0).getOffences().get(0).getCommittingCourt().isPresent(),
                is(false));
    }

    @Test
    void shouldMapAllAddressFieldsIncludingWelshViaConvert() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearingWithWelshAddress();

        final uk.gov.moj.cpp.listing.domain.Hearing actual = commandToDomainConverter.convert(commandHearing);

        final uk.gov.justice.core.courts.Address commandAddress =
                commandHearing.getProsecutionCases().get(0).getDefendants().get(0)
                        .getPersonDefendant().getPersonDetails().getAddress();
        final uk.gov.moj.cpp.listing.domain.Address domainAddress =
                actual.getListedCases().get(0).getDefendants().get(0).getAddress().get();

        assertThat(domainAddress.getAddress1(), is(commandAddress.getAddress1()));
        assertThat(domainAddress.getAddress2().get(), is(commandAddress.getAddress2()));
        assertThat(domainAddress.getAddress3().get(), is(commandAddress.getAddress3()));
        assertThat(domainAddress.getAddress4().get(), is(commandAddress.getAddress4()));
        assertThat(domainAddress.getAddress5().get(), is(commandAddress.getAddress5()));
        assertThat(domainAddress.getPostcode().get(), is(commandAddress.getPostcode()));
        assertThat(domainAddress.getWelshAddress1().get(), is(commandAddress.getWelshAddress1()));
        assertThat(domainAddress.getWelshAddress2().get(), is(commandAddress.getWelshAddress2()));
        assertThat(domainAddress.getWelshAddress3().get(), is(commandAddress.getWelshAddress3()));
        assertThat(domainAddress.getWelshAddress4().get(), is(commandAddress.getWelshAddress4()));
        assertThat(domainAddress.getWelshAddress5().get(), is(commandAddress.getWelshAddress5()));
    }

    @Test
    void shouldMapNullableAddressFieldsAsEmptyOptionalViaConvert() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearing();
        ...
        assertThat(domainAddress.getWelshAddress1().isPresent(), is(false));
        assertThat(domainAddress.getWelshAddress2().isPresent(), is(false));
        assertThat(domainAddress.getWelshAddress3().isPresent(), is(false));
        assertThat(domainAddress.getWelshAddress4().isPresent(), is(false));
        assertThat(domainAddress.getWelshAddress5().isPresent(), is(false));
    }

    @Test
    void shouldBuildLaaReferenceWhenLaaApplnReferenceIsPresent() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearingWithLaaReference();

        final uk.gov.moj.cpp.listing.domain.Hearing actual = commandToDomainConverter.convert(commandHearing);

        final uk.gov.justice.core.courts.LaaReference commandLaaRef =
                commandHearing.getProsecutionCases().get(0).getDefendants().get(0).getOffences().get(0).getLaaApplnReference();
        final LaaReference domainLaaRef =
                actual.getListedCases().get(0).getDefendants().get(0).getOffences().get(0).getLaaApplnReference().get();

        assertThat(domainLaaRef.getApplicationReference(), is(commandLaaRef.getApplicationReference()));
        assertThat(domainLaaRef.getStatusCode(), is(commandLaaRef.getStatusCode()));
        assertThat(domainLaaRef.getStatusDate(), is(commandLaaRef.getStatusDate()));
        assertThat(domainLaaRef.getStatusDescription(), is(commandLaaRef.getStatusDescription()));
        assertThat(domainLaaRef.getStatusId(), is(commandLaaRef.getStatusId()));
        assertThat(domainLaaRef.getEffectiveStartDate(), is(of(commandLaaRef.getEffectiveStartDate())));
        assertThat(domainLaaRef.getEffectiveEndDate(), is(of(commandLaaRef.getEffectiveEndDate())));
    }

    @Test
    void shouldReturnEmptyOptionalWhenLaaApplnReferenceIsNull() {
        final HearingListingNeeds commandHearing = commandBuilder.buildCommandHearing();

        final uk.gov.moj.cpp.listing.domain.Hearing actual = commandToDomainConverter.convert(commandHearing);

        assertThat(actual.getListedCases().get(0).getDefendants().get(0).getOffences().get(0).getLaaApplnReference(),
                is(empty()));
    }
