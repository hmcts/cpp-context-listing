...
import static uk.gov.moj.cpp.listing.persistence.repository.courtlist.HearingJdbcRepository.NULL_AMP_PUBLIC_DATA_LAST_UPDATED;
...
public abstract class HearingRepository implements EntityRepository<Hearing, UUID>, EntityManagerDelegate<Hearing> {
    ...
    @Query(value = "select h.id, h.properties,  " +
            ...
            " ASC "
            , isNative = true)
    public abstract List<Hearing> findHearingsByWeekCommencingRangeWithNoPagination(
            final String courtCentreId,
            final String courtRoomId,
            final String authorityCode,
            final String hearingTypeId,
     ...
     * @return Hearings.
     */
    @Query(value = UNALLOCATED_COMMON_SELECT_FROM +
            "where  " +
            "(h.is_vacated_trial is null or h.is_vacated_trial != true) and " +
            "h.allocated = ?8  " +
            "and " +
    ...
    @Query(value = UNALLOCATED_COMMON_SELECT_FROM +
            ...
            "and h.allocated = ?8  " +
            "and h.is_possible_disqualification = ?9  " +
            "and (?1 is null or h.court_centre_id = cast(cast(?1 as varchar) as uuid))  " +
            ...
            , isNative = true)
    public abstract List<Hearing> findUnallocatedHearingsByWeekCommencingRangeAndPossibleDisqualification(
            final String courtCentreId,
            final String courtRoomId,
            final String authorityCode,
            final String hearingTypeId,
     ...
     * @return
     */
    @Query(value = "select id, properties, amp_public_data_last_updated, " +
            ...
            "null as is_possible_disqualification, " + NULL_FLAT_HEARING_FIELDS +
            " from ( " +
            " select distinct h.id as id, h.properties as properties, h.start_date as startDate, h.end_date as endDate, h.amp_public_data_last_updated " +
            " from hearing h " +
            " LEFT JOIN listed_cases lc ON lc.hearing_id = h.id " +
